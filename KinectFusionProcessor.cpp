...
KinectFusionProcessor::KinectFusionProcessor() :
    ...
    m_MarkerBoundCoordinates(nullptr),//TODO:marker
    m_pMapper(nullptr),
    ...
    m_bHaveValidCameraParameters(false)
{
    ...
    //TODO:marker
    marker.readParameters("marker.yml", cColorWidth, cColorHeight);
    marker.initialize();
}
...
KinectFusionProcessor::~KinectFusionProcessor()
{
    ...
    //TODO:marker
    SAFE_DELETE_ARRAY(m_MarkerBoundCoordinates);
...
HRESULT KinectFusionProcessor::InitializeKinectFusion()
{
    ...
    const UINT colorBufferSize = colorWidth * colorHeight;//TODO:marker
    ...
    //TODO:marker
    SAFE_DELETE_ARRAY(m_MarkerBoundCoordinates);
    m_MarkerBoundCoordinates = new CameraSpacePoint[colorBufferSize];
    if (nullptr == m_MarkerBoundCoordinates)
    {
        SetStatusMessage(L"Failed to initialize Kinect bounding coordinate buffer.");
        return E_OUTOFMEMORY;
    }
...
HRESULT KinectFusionProcessor::MapColorToDepth(Point2i* m)
{
    HRESULT hr = S_OK;
    ...
    const UINT brange = 30; // TODO:marker
                ...
                if(mappedIndex < depthImagePixels)
                {
                    ...
                    if (colorX < colorWidth && colorY < colorHeight)
                    {
                        ...
                        if(depthValue - depthTestValue < cDepthVisibilityTestThreshold)
                        {
                            ...
                            bool paint = false; // TODO:marker, test, need to false
                            for (int i = 0; i < 4; i++)
                            {
                                if ((m[i].x - brange < colorX && colorX < m[i].x + brange) &&
                                    (m[i].y - brange < colorY && colorY < m[i].y + brange))
                                {
                                    paint = true;
                                    break;
                                }
...
bool KinectFusionProcessor::ProcessDepth()
{
    ...
    if (integrateData)
    {
        ...
        {
            // TODO:marker
            Mat kinectframe(Size(cColorWidth, cColorHeight), CV_8UC4, m_pColorImage->pFrameBuffer->pBits); 
            cvtColor(kinectframe, kinectframe, CV_BGRA2BGR);
            flip(kinectframe, kinectframe, 1); // m_paramsCurrent.m_bMirrorDepthFrame, marker mirror will not detected

            std::vector<cv::Point2f> bounding = marker.markerDetect(kinectframe); // maybe marker detect is not found
            m_pMapper->MapColorFrameToCameraSpace(NUI_DEPTH_RAW_WIDTH * NUI_DEPTH_RAW_HEIGHT, m_pDepthRawPixelBuffer, cColorWidth * cColorHeight, m_MarkerBoundCoordinates); 
            cout << "\nKinect corners Round:\n";
            
            Point2i m[4];
            int i = 0;
            for (std::vector<cv::Point2f>::iterator it = bounding.begin(); it != bounding.end(); ++it, ++i) {
                //
                m[i].x = cColorWidth - clamp((int)round(it->x), 0, cColorWidth - 1); // TODO:recover horizontal flip, AruMarker class handle is better
                m[i].y = clamp((int)round(it->y), 0, cColorHeight - 1);
                int pos = m[i].x + m[i].y * cColorWidth;
                std::cout << pos << " : [" << m_MarkerBoundCoordinates[pos].X << " , " << m_MarkerBoundCoordinates[pos].Y << " , " << m_MarkerBoundCoordinates[pos].Z << "]";
                cout << endl;
            }
